...
def train(opt, train_loader, test_loader, val_loader, board, tocg, D):
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step)):
        ...
        cm_paired = torch.FloatTensor((cm_paired.detach().cpu().numpy() > 0.5).astype(float)).cuda() # NOTE: unnecessary since cm_paired is already onehot array. See CPDataset. 
        # input2
        ...
        if opt.interflowloss:
            for i in range(len(flow_list)-1):
                ...
                # Note: compare with how flow_norm is computed in ConditionGenerator, though look different, but the are using the same formula; since fW and fH are size of original flow, and not need to use fH/2 or fH/2 as in `ConditionGenerator.forward`. Values of flow_norm is also doubled to accelerate changes just as in `ConditionGenerator.forward`. 
